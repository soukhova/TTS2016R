...
load(file=paste0(here::here(),"/data-raw/TTS16-data-inputs/gamma_.Rdata"))
```
...
```{r TLD-Gamma-plot, fig.height=6, eval=FALSE}
plot(gamma_)
```

```{r TLD-Gamma-plot-load, include=FALSE}
# png(file=paste0(here::here(),"/data-raw/TTS16-data-inputs/gamma_plot.png"))
# plot(gamma_)
# dev.off() 
knitr::include_graphics(paste0(here::here(),"/data-raw/TTS16-data-inputs/gamma_plot.png"))
```
...
```{r create-cullen-frey-data, message = FALSE, echo=FALSE, fig.height=6, eval=FALSE}
...
```{r create-cullen-frey-data-load, include=FALSE}
# od  <- od %>% filter( !is.na(travel_time)) %>% mutate(travel_time = ifelse(travel_time == 0, 0.1, travel_time))
# all_tt <- od  %>% dplyr::select(Persons, travel_time)
# all_tt <- all_tt[rep(seq_len(dim(all_tt)[1]), all_tt$Persons), 2]
...
# fitdistrplus::descdist(data=all_tt) 
# dev.off() 
knitr::include_graphics(paste0(here::here(),"/data-raw/TTS16-data-inputs/cullen-frey_plot.png"))
```
