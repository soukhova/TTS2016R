...
This manuscript presents the open data product [{TTS2016R}](https://github.com/soukhova/TTS2016R). Open data products are the result of turning source data (open or otherwise) into accessible information that adds value to the original inputs [see @Arribas2021open]. The product presented in this paper is an `R` data package which currently consists of three objects which are sourced from the 2016 Transportation Tomorrow Survey (TTS) or are curated to facilitate the use and analysis of TTS data. This package includes full-time home-based work-to-job origin destinations counts and mode-specific trip numbers, traffic analysis zone (TAZ) boundaries, and planning/municipality boundaries for the Greater Golden Horse area (GGH) located in southern Ontario, Canada [@data_management_group_tts_2018]. In addition, the package includes TAZ centroid-to-centroid travel times by car, transit, cyling, and walking mode computed using package {r5r} [@Pereira2021r5r]. 

The aim of this paper is to walk readers through the empirical home-based work commute data set, illustrate the calculation of an impedance function that can be used to calculate accessibility to employment, and invite its use in other applications. Though data from the TTS is freely available to the public (URL <!-- -->), the raw data can be technically demanding, cumbersome to work with, and requires multiple software to process. By pre-processing the data in a `R` environment, {TTS2016R} offers a slice of the TTS data useful to understand patterns of commuting to work in the region and amends calculated travel times to the data set. This package also provides open infrastructure for additional TTS or complimentary data sets to be amended by the authors or wider open-source community in the future. 
...
{TTS2016R} includes counts of full-time employed population by place of residence (origin), counts of full-time usual place of work (destination), number of trips to work by mode, average estimated travel time of trip, and the number of vehicles by mode. This data is aggregated and available at the level of TAZ: TAZ are a spatial unit of analysis typically used to estimate the number of trips produced and attracted to each zone [@meyer_urban_2001]. They are thus defined by transportation planners for a region based on intra-similarity and inter-dissimilarity between land-use and population demographics. Within the GGH boundaries, `r round(length(TTS2016R::ggh_taz$GTA06), 3) %>% prettyNum(big.mark = ",")` TAZ are specified. The TAZ range between ≥`r 0.019` km^2 in spatial area to a maximum of `r max(TTS2016R::ggh_taz$AREA) %>% prettyNum(digits =2, big.mark = ",")` km^2 (median: `r median(TTS2016R::ggh_taz$AREA) %>% prettyNum(digits =2, big.mark = ",")` km^2 and 3rd quantile: `r quantile(TTS2016R::ggh_taz$AREA, probs=0.75) %>% unname() %>% prettyNum(digits =2, big.mark = ",")` km^2). 

The number of full-time working population (will be referred to as 'workers' throughout) and full-time places of employment (will be referred to as 'jobs' throughout) is also quite variable between TAZ. In terms of workers, TAZ contain between `r min(TTS2016R::ggh_taz$workers) %>% prettyNum(digits =2, big.mark = ",")` to `r max(TTS2016R::ggh_taz$workers) %>% prettyNum(digits =2, big.mark = ",")` workers (median: `r median(TTS2016R::ggh_taz$workers) %>% prettyNum(digits =2, big.mark = ",")`, 3rd quantile: `r quantile(TTS2016R::ggh_taz$workers, probs=0.75) %>% unname() %>% prettyNum(digits =2, big.mark = ",")`). In terms of jobs, TAZ contain between `r min(TTS2016R::ggh_taz$jobs) %>% prettyNum(digits =2, big.mark = ",")` to `r max(TTS2016R::ggh_taz$jobs) %>% prettyNum(digits =2, big.mark = ",")` jobs (median: `r median(TTS2016R::ggh_taz$jobs) %>% prettyNum(digits =2, big.mark = ",")`, 3rd quantile: `r quantile(TTS2016R::ggh_taz$jobs, probs=0.75) %>% unname() %>% prettyNum(digits =2, big.mark = ",")`).

Each TAZ is uniquely identified using the GTA06 Zoning System which can be used to match the worker residential location to the job location in the origin-destination table. The number of jobs (`r round(sum(TTS2016R::ggh_taz$jobs), 3) %>% prettyNum(big.mark = ",")`) and workers (`r round(sum(TTS2016R::ggh_taz$workers), 3) %>% prettyNum(big.mark = ",")`) in this package are organized in the form of an origin-destination table which is indicative of home-to-work commute patterns (there are `r round(sum(TTS2016R::od_ft_tt$trips), 3) %>% prettyNum(big.mark = ",")` potential interactions). This data was retrieved from the TTS Data Retrieval System on October 28, 2021 and reflect the interaction of full-time employed people to their places of employment as shown in Figure \ref{fig:TTS-16-survey-area} as defined by the 2016 TTS methodology [@data_management_group_tts_2018].

Also included in {TTS2016R} are the estimated travel times from origin to destination by various modes; travel times are calculated using the package {r5r} and the notebook for this calculation is availalbe in the documentation of {TTS2016R}. These travel times are useful addition to {TTS2016R} since they are not included in the TTS Data Retrieval System but they are vitally important to estimate the cost of travel and associated impedance functions, among other possible applications. The included travel times are by car, walking, cyling, and public transit modes and are calculated from each origin TAZ centroid to each destination TAZ centroid where a home-based work trip is made (`r TTS2016R::od_ft_tt$Origin %>% length() %>% prettyNum(big.mark = ",") ` OD pairs). The centroid is snapped to the nearest street line by `r5r` and the travel time is calculated for all pairs. Additionally, for car and transit modes only travel times less than or equal to 180 mins are calculated. For cycling and walking modes only travel times less than or equal to 120 mins are calculated. 

These threshold represents 90% or greater of trip's travel times, per mode, which are summarized in Figure \ref{fig:TTS-16-survey-area}.
