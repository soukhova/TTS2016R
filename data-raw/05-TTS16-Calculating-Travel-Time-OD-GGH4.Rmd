...
library(disk.frame) # not installed on this machine
...
library(progress) # Terminal Progress Bars
library(purrr) # Functional Programming Tools
library(sf) # Simple Features for R
library(stplanr) # not installed on this machine
library(tidycensus) # not installed on this machine
...
options(future.globals.maxSize = Inf) # allow disk.frame threads to talk to each other without limits
tmap_mode("plot")
```


```{r}
options(java.parameters = "-Xmx30G") # assign memory for r5r to work with
library(r5r) # Rapid Realistic Routing with 'R5' 
```

##--- Prepare Input Data

Prepare destination points and origins points:
```{r}
data("ggh_taz_v4")
...
work_origins <- ggh_taz_v4 |>
  st_drop_geometry() |>
  transmute(id = GGHV4,
            lon = coords[,1],
            lat = coords[,2])

job_destinations <- work_origins
```
...
Version of data downloaded:

```{r download data, include=FALSE, eval=FALSE}
#downloading ontario osm in the correct format
download.file(url = paste0("https://download.openstreetmap.fr/extracts/north-america/canada/ontario/golden_horseshoe-latest.osm.pbf"),
...
<!-- QUESTION FOR RAFA ET AL. IS IT POSSIBLE TO HAVE MULTIPLE osm.pbf FILES IN THE SAME DIRECTORY AND CALL THEM BY NAME? -->
