...
for(p in 1:length(purpose)){
  ...
  for(m in 1:length(mode)){
    ...
    for(d in 1:length(weekday)){
      ...
      raw_data <- read_delim(file = glue("ttsCross-OD-GGH4-{purpose[p]}-{weekday[d]}-{mode[m]}.txt"), 
                             delim = "\t", 
                             col_names = FALSE,
  ...
  for(m in 1:length(mode)){
    ...
    for(d in 1:length(weekday)){
      ...
      if(length(idx) > 1){
        ...
        print(glue("Table ttsCross-OD-GGH4-{purpose[p]}-{weekday[d]}-{mode[m]}.txt is empty: check"))
...
for(t in 1:nrow(empty_tables)){
  # Read data file for purpose/weekday
  raw_data <- read_delim(file = glue("ttsCross-OD-GGH4-{empty_tables$prps[t]}-{empty_tables$day[t]}-{empty_tables$md[t]}.txt"), 
                         delim = "\t", 
                         col_names = FALSE,
                         show_col_types = FALSE,
  ...
  if(length(idx) > 1){
    ...
    for(i in 1:(length(idx) - 1)){
      ...
      od_purpose_temp2 <- read_csv(glue("ttsCross-OD-GGH4-{empty_tables$prps[t]}-{empty_tables$day[t]}-{empty_tables$md[t]}.txt"),
                                   skip = idx[i] + 1,
