...
#library(logspline) # not installed on this machine
library(readr) # not installed on this machine
library(sf) # Simple Features for R
library(stringr)
library(tidyr)
#library(tidyverse) # not installed on this machine
...
```{r}
ggh_pd <- ggh_pd |>
  mutate(REGION_name = case_when(REGION == 1 ~ "Toronto",
                                  REGION == 2 ~ "Durham",
                                  REGION == 3 ~ "York",
                                  REGION == 4 ~ "Peel",
                                  REGION == 5 ~ "Halton",
                                  ...
                                  REGION == 12 ~ "Waterloo",
                                  REGION == 13 ~ "Guelph",
                                  REGION == 14 ~ "Wellington",
                                  REGION == 15 ~ "Orangeville",
                                  REGION == 16 ~ "Barrie",
                                  REGION == 17 ~ "Simcoe",
                                  REGION == 18 ~ "Kawartha Lakes",
                                  REGION == 19 ~ "Peterborough City",
                                  REGION == 20 ~ "Peterborough County", 
                                  REGION == 21 ~ "Orillia", 
                                  REGION == 22 ~ "Dufferin",
                                  REGION == 23 ~ "Brantford",
                                  REGION == 24 ~ "Brant"))
...
ggh_taz <- st_buffer(ggh_taz, 
                     dist = 0)

ggh_pd <- st_buffer(ggh_pd, 
                    dist = 0)

ggh_taz <- st_join(ggh_taz, 
                   ggh_pd |>
                     transmute(REGION),
                   left = TRUE, 
                   largest = TRUE)
